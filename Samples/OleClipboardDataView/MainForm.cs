...
internal sealed partial class MainForm : Form
{
		...
		if (dataobj != null)
		{
			foreach (var fmtetc in dataobj.FormatEtcGetterEnumerable)
			{
				...
				var item = listView1.Items.Add(new ListViewItem([
					...
					fmtetc.Index.ToString(),
					fmtetc.TargetDevicePointer.ToString(),
					medium.Tymed.ToString(),
					medium.GetByteLength().ToString(),
					medium.UnknownPointerForRelease.ToString(),
				]));
				item.Tag = medium;
			}
		}
	...
	private void listView1_SelectedIndexChanged(object sender, EventArgs e)
	{
		...
		var medium = (ComStorageMedium)listView1.SelectedItems[0].Tag;

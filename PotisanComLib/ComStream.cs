...
public class ComStream(object? o) : ComUnknownWrapperBase<IStream>(o), ICloneable
{
	...
	public static ComResult<ComStream> CreateOnFileNoThrow(string path, STGM mode, bool creates, uint attrs = 0, ComStream? template = null)
	{
	...
	public static ComStream CreateOnFile(string path, STGM mode, bool creates, uint attrs = 0, ComStream? template = null)
		=> CreateOnFileNoThrow(path, mode, creates, attrs, template).Value;

	public static ComResult<ComStream> OpenFileNoThrow(string path, STGM mode = STGM.Read | STGM.ShareDenyWrite)
		=> CreateOnFileNoThrow(path, mode, false, 0, null);

	public static ComStream OpenFile(string path, STGM mode = STGM.Read | STGM.ShareDenyWrite)
...
/// </summary>
/// <remarks>
/// STGMはWindows SDKで慣用的に使用されるのでそのままの名前としています。
/// </remarks>
[Flags]

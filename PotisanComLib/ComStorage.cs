...
{
	public ComResult<ComStream> CreateStreamNoThrow(string name, STGM mode)
		=> new(_obj.CreateStream(name, (uint)mode, 0, 0, out var x), new(x));

	public ComStream CreateStream(string name, STGM mode = STGM.ReadWrite | STGM.ShareExclusive)
		=> CreateStreamNoThrow(name, mode).Value;

	public ComResult<ComStream> OpenStreamNoThrow(string name, STGM mode = STGM.ReadWrite | STGM.ShareExclusive)
		=> new(_obj.OpenStream(name, 0, (uint)mode, 0, out var x), new(x));

	public ComStream OpenStream(string name, STGM mode)
		=> OpenStreamNoThrow(name, mode).Value;

	public ComResult<ComStorage> CreateStorageNoThrow(string name, STGM mode)
		=> new(_obj.CreateStorage(name, (uint)mode, 0, 0, out var x), new(x));

	public ComStorage CreateStorage(string name, STGM mode)
		=> CreateStorageNoThrow(name, mode).Value;

	public ComResult<ComStorage> OpenStorageNoThrow(string name, STGM mode, ReadOnlySpan<string> excludes)
	{
	...
	public ComStorage OpenStorage(string name, STGM mode, ReadOnlySpan<string> excludes)
		=> OpenStorageNoThrow(name, mode, excludes).Value;

	public ComResult<ComStorage> OpenStorageNoThrow(string name, STGM mode)
		=> OpenStorageNoThrow(name, mode, []);

	public ComStorage OpenStorage(string name, STGM mode)
	...
	public static ComResult<ComStorage> CreateFileNoThrow(string name, STGM mode, ComStorageFormat format)
	{
	...
	public static ComStorage CreateFile(string name, STGM mode = STGM.ReadWrite | STGM.ShareExclusive, ComStorageFormat format = ComStorageFormat.Storage)
		=> CreateFileNoThrow(name, mode, format).Value;

	public static ComResult<ComStorage> OpenFileNoThrow(string name, STGM mode = STGM.ReadWrite | STGM.ShareExclusive, ComStorageFormat format = ComStorageFormat.Storage)
	{
		[DllImport("ole32.dll", CharSet = CharSet.Unicode)]
	...
	public static ComStorage OpenFile(string name, STGM mode, ComStorageFormat format)
